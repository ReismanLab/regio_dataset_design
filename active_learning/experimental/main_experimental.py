...
parser.add_argument('--yields', type=int, nargs='+',
                    help='List of experimental yields')
...
if args.sels:
    ...
    yields       = [int(y) for y in args.yields]
    sels = None
else:
    assert False, "Must provide yields or selectivities"
acqf             = args.acqf
...
for col in df.columns:
    if col == "DOI":
        continue
    if col in ["Reactant_SMILES", "Atom_nº", "Reactive Atom", "Selectivity"]:
