...
parser.add_argument('--aqcf',
                    help='List of the aquisition functions to be used',
                    default=['random', 
...
### util data:
df = pd.read_csv(f"{base_cwd}/data/descriptors/preprocessed_reactions_no_unspec_no_intra/df_bde.csv", index_col=0)

## load data for random training:
df_bde = pd.read_csv(f"{base_cwd}/data/descriptors/preprocessed_reactions_no_unspec_no_intra/df_bde.csv", index_col=0)
df_xtb = pd.read_csv(f"{base_cwd}/data/descriptors/preprocessed_reactions_no_unspec_no_intra/df_xtb.csv", index_col=0)
df_gas = pd.read_csv(f"{base_cwd}/data/descriptors/preprocessed_reactions_no_unspec_no_intra/df_gas.csv", index_col=0)
...
def plot_summary(smiles, t5, df, _, carbo_preds, save=True, labels=['Top1', 'Top2', 'Top3', 'Top5', 'Top10'], aqcf_list=aqcf_list):
    ...
    #dict_name = {0: 'Random',
    #             1: 'ACQF_1', 2: 'ACQF_2', 3: 'ACQF_3', 
    #             4: 'ACQF_4', 5: 'ACQF_5', 6: 'ACQF_6', 
    #             7: 'ACQF_7', 8: 'ACQF_8', 9: 'ACQF_9'}  
...
for smiles_1 in list(select_files.keys()):
    ...
    #m                       = get_metrics(df_bde, smiles_1, carbon_preds_)
    #if m == None:
    #    continue

    plot_summary(smiles_1, t5, df, _, carbon_preds_)#, labels=['Top1', 'M2', 'M3', 'M4', 'MAE', 'MAE-exp', 'RMSE'])
    #plot_summary(smiles_1, m, df, train_smiles, reg, _, carbon_preds_)#, labels=['Top1', 'M2', 'M3', 'M4', 'MAE', 'MAE-exp', 'RMSE'])
    _ += 1
