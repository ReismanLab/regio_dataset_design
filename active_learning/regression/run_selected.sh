...
for smi in ${smis[@]}; do
  for acqf in ${acqfs[@]}; do
    for idx in ${n_runs[@]}; do #$(seq$n_runs); do
        python ./main.py --smi $smi --acqf $acqf --batch $b --start $start --n_repet $n_repet --db $db --feat $feat --n_est $n_est --max_feats $max_feat --max_depth $max_depth --min_samples_leaf $min_samples --model $model --selection_strat $strat --res $res --run $idx --df_folder $folder --alpha $alpha --y $obs
    done
  done
done
